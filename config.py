...
class SystemConfig:
    ...
    def _apply_settings(self, settings: Dict[str, Any]):
        ...
        # Region of Interest
        # ROI_TOP_RATIO=0.60 gives a consistent 40% strip above the mid-line,
        # ignoring sky / distant background without cutting off near-lane markings.
        ...
        # Hough Transform
        # Lower threshold (20) and shorter minLineLength (15) with a larger
        # maxLineGap (50) improve detection on faded / broken lane markings.
        self.HOUGH_RHO             = lane.get('hough_rho', 2)
        self.HOUGH_THETA           = lane.get('hough_theta', 1)
        self.HOUGH_THRESHOLD       = lane.get('hough_threshold', 20)
        self.HOUGH_MIN_LINE_LENGTH = lane.get('hough_min_line_length', 15)
        ...
        self.BASE_SPEED = control.get('base_speed', 25)
        self.MAX_SPEED  = control.get('max_speed', 100)
        self.MIN_SPEED  = control.get('min_speed', 10)

        # Steering
        # STEER_KP=0.20  — proportional gain. At 30px offset this produces a
        #   6-degree correction; previous 0.5 produced 15 degrees causing
        #   oscillation and lane drift.
        # STEER_SMOOTHING=0.5 — blends previous and new steering angle each
        #   frame to damp jitter: angle = prev*(1-smoothing) + new*smoothing.
        # STEER_TRIM — static trim offset applied after clamping to compensate
        #   for physical wheel misalignment. Tune in ±1 steps:
        ...
        self.STEER_KP         = control.get('steer_kp', 0.20)
        ...
        self.LOG_TELEMETRY     = logging_cfg.get('enable_telemetry', True)
        self.LOG_DIRECTORY     = logging_cfg.get('directory', 'test_logs')
        self.FRAME_LOG_INTERVAL = logging_cfg.get('frame_interval', 30)
    ...
    def _get_default_settings() -> Dict[str, Any]:
        ...
        These values are the single source of truth — _apply_settings
        uses these same values as its fallback defaults, so the two are
        always in sync.
        """
        return {
            "performance": {
    ...
    def print_summary(self):
        ...
        print(f"  Min line length:   {self.HOUGH_MIN_LINE_LENGTH}px")
        print(f"  Max line gap:      {self.HOUGH_MAX_LINE_GAP}px")

        print(f"\n[OBSTACLE DETECTION - Blob Detection]")
        print(f"  Safety Zone:       {self.SAFETY_ZONE_WIDTH_RATIO*100:.0f}% width x {self.SAFETY_ZONE_HEIGHT_RATIO*100:.0f}% height")
        ...
        print(f"  Steering Range:    +/-{self.MAX_STEER_ANGLE} deg")
        print(f"  Steer Kp:          {self.STEER_KP}")
        print(f"  Steer Smoothing:   {self.STEER_SMOOTHING}")
        print(f"  Steer Trim:        {self.STEER_TRIM:+d} deg ({'right' if self.STEER_TRIM > 0 else 'left' if self.STEER_TRIM < 0 else 'centre'})")
...
# Automatically loads from settings.json if present, otherwise uses defaults.
config = SystemConfig()


# ============================================================================
# BACKWARD COMPATIBILITY - Direct module-level variable access
# ============================================================================
# Supports both "from config import STEER_KP" and "import config; config.STEER_KP"
...
STEER_TRIM       = config.STEER_TRIM       # NEW — static wheel alignment trim
...
if __name__ == "__main__":
    ...
    print("\n[INFO] Rename to settings.json to activate custom configuration\n")
